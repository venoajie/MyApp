...
async def time_stamp_to_recover(my_trades_from_db_recovery) -> list:
    ...
    return (
        []
        if my_trades_from_db_recovery in none_data
...
async def myTrades_originally_from_db(currency) -> list:
    ...
    try:
        my_trades_path_open_recovery = system_tools.provide_path_for_file(
            "myTrades", currency, "open-recovery-point"
        )
        ...
        return {
            "db_recover": my_trades_from_db_recovery,
            "time_stamp_to_recover": await time_stamp_to_recover(
...
class CheckTradeIntegrity:
    ...
    async def compare_inventory_per_db_vs_get(self) -> int:
        ...
        try:
            ...
            log.critical(
                f"size_from_get_db {size_from_get_db} size_from_trading_db {size_from_trading_db}"
            )
    ...
    async def rebuilt_db_myTrades_open(self) -> None:
        ...
        try:
            ...
            # log.debug(my_trades_from_db_recovery)
            ...
            if my_trades_from_db_recovery:
                ...
                filtered_data_from_my_trades_from_exchange = (
                    string_modification.find_unique_elements(
                        self.my_selected_trades_open_from_system,
                        my_trades_from_db_recovery,
                    )
                )
                log.error(
                    f"filtered_data_from_my_trades_from_exchange {filtered_data_from_my_trades_from_exchange}"
                )
                ...
                my_trades_from_exchange_filtered.distribute_trade_transactions(
                    self.currency, True
                )
    ...
    async def update_myTrades_file_as_per_comparation_result(self) -> list:
        """ """
        try:
            ...
            if size_difference == 0:
                ...
                pickling.replace_data(
                    my_trades_path_open["path_recover"],
...
async def main_enforce_my_trade_db_integrity(
    ...
    trade_integrity = CheckTradeIntegrity(
        currency,
        positions_from_get,
        my_trades_open_from_db,
        my_selected_trades_open_from_system,
    ...
    async def compare_open_order_per_db_vs_get(self) -> int:
        ...
        try:
            ...
            both_sources_are_equivalent = (
                self.open_order_from_get == self.open_order_from_db
            )
            log.critical(
                f"both_sources_are_equivalent {both_sources_are_equivalent} open_order_from_get {self.open_order_from_get} open_order_from_db {self. open_order_from_db}"
            )
