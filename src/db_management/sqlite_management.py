...
def querying_ohlc_closed(
    ...
    all_data= f"""SELECT tick, JSON_EXTRACT (data, '$.volume') AS volume, JSON_EXTRACT (data, '$.{price}')  AS close FROM {table}"""

    return all_data  if limit== None else \
        f"""SELECT tick, JSON_EXTRACT (data, '$.volume') AS volume, JSON_EXTRACT (data, '$.{price}')  AS close FROM {table} ORDER BY tick DESC limit {limit}"""
