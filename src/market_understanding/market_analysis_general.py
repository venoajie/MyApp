...
class Options ():
    ...
    def time_to_expiration (self)-> float:
        ...
        transaction_time_utc = formula.convert_time_to_utc (self.expiration_date)['transaction_time']
        now_time_utc = formula.convert_time_to_utc ()['utc_now']
        diff_time_seconds = (transaction_time_utc - now_time_utc).total_seconds() #https://stackoverflow.com/questions/4362491/how-do-i-check-the-difference-in-seconds-between-two-dates
        diff_time_hours = diff_time_seconds/3600
        return (diff_time_hours/24)
        
    def compute_implied_volatility (self, flag: str, model: str = 'black_scholes')-> list:
            
        import py_vollib_vectorized
        if model == 'black_scholes':    
            return py_vollib_vectorized.vectorized_implied_volatility_black  (self.option_price, 
    ...
    def compute_price (self, flag:str, model: str = 'black_scholes')-> list:
        ...
        log.warning (f'{flag=} {self.option_price=} {self.strike=} {self.time_to_expiration ()=} {self.interest=} ')
        implied_vloatility = self. compute_implied_volatility(flag, model)

        return py_vollib.black_scholes.black_scholes(flag, 
                                                     self.option_price, 
                                                     self.strike, 
                                                     self.time_to_expiration (),
                                                     self.interest, 
    ...
    def get_all_greeks (self, flag: str, model: str, return_as: str)-> dict:
        ...
        return get_all_greeks(flag, 
                              self.option_price, 
                              self.strike, 
                              self.time_to_expiration (), 
                              self.interest, 
                              implied_vloatility, model=model, return_as= return_as) 
    ...
    def get_all_greeks_df (self, df):
        ...
        return price_dataframe(df, flag_col='Flag', underlying_price_col='S', strike_col='K', annualized_tte_col='T',
                        riskfree_rate_col='R', sigma_col='IV', model='black_scholes', inplace=False)
    
def is_contango_or_backwardation (futures_price: float, 
                                  spot_or_perpetual_price: float
                                  )-> Dict:
    
    margin = futures_price - spot_or_perpetual_price
    
    return {'margin_usd': [] if futures_price==[] else margin,
...
def fetch_and_combine_funding_rate_with_heatmap_result (dataframe):
    ...
    try:
        ...
        dataframe ['nextFundingRate'] = [(fetch_next_funding.fetch_next_funding_coin ((o))[
            'nextFundingRate']) for o in dataframe['future']]
...
def fetch_correlation_from_sqlite_  (threshold)->str:
    ...
    return  {'correl_above_threshold': correl_above_threshold ,
                'correl_all': correl,
                'correl_below_threshold': correl_below_threshold,
                'above_threshold_rate_positive': [o for o in [ o for o in correl_above_threshold if o ['nextFundingRate'] > 0]] ,
                'above_threshold_rate_negative': [o for o in [ o for o in correl_above_threshold if o ['nextFundingRate'] < 0]] }
...
@unsync
def heatmap_and_funding_rate_ (symbols: list, 
    ...
    try:
       ...
       # ohlc_mid_main = pd.Series(ohlc_df['close'], dtype='float32')
        ...
        for symbol in symbols:
            ...
            symbol_underlying = formula.transform_underlying_from_or_to_perpetual (symbol, 'underlying')
            symbol = formula.transform_underlying_from_or_to_perpetual (symbol, 'perpetual')
    ...
    return {'all_result': coeffs,
            'correl_above_threshold': correl_above_threshold  ,
            'correl_below_threshold': correl_below_threshold,
            'above_threshold_rate_positive': correl_above_threshold[correl_above_threshold['nextFundingRate'] > 0],
            'above_threshold_rate_negative': correl_above_threshold[correl_above_threshold['nextFundingRate'] < 0] }

def heatmap_and_funding_rate (symbols: list, 
                               main_underlying: str = 'BTC', 
                               time_frame: int = 3600, 
                               threshold: float = 0
                              ):

    heatmap_and_funding_rate = heatmap_and_funding_rate_ (symbols,
                                                          main_underlying, 
                                                          time_frame, 
                                                          threshold)
