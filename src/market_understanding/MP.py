...
class MpFunctions:
    ...
    def get_rf(self):
        self.data["cup"] = np.where(
            self.data["Close"] >= self.data["Close"].shift(), 1, -1
        )
        self.data["hup"] = np.where(
            self.data["High"] >= self.data["High"].shift(), 1, -1
        )
    ...
    def get_mean(self):
        ...
        rf30 = (
            abs((dfhistd["rf"])).rolling(self.avglen).mean()
    ...
    def tpo(self, dft_rs):
        ...
        if len(dft_rs) > int(0):
            ...
            tpoval = dftpo[self.ticksize : -(self.ticksize)][
                "tpocount"
            ]  # take mid section
    ...
    def get_dayrank(self):
        ...
        dist_df["vold_zscore"] = (dist_df.volumed - vol_mean) / dist_df.volumed.std(
            ddof=0
        )
        dist_df["rfd_zscore"] = (abs(dist_df.rfd) - rf_mean) / abs(dist_df.rfd).std(
            ddof=0
        )
        a, b = 1, 4
