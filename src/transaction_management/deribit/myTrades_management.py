...
class MyTrades:
    ...
    def my_trades_api_basedOn_label(self, label: str) -> list:
        ...
        return (
            []
            if self.my_trades_api() == []
            else [o for o in self.my_trades_api() if label in o["label"]]
        )

    def net_sum_trade_size(self, selected_trades: list = None) -> float:
        """ """
    ...
    def recognize_trade_transactions(self, trade_order: list) -> dict:
        """ """
    ...
    def gather_transactions_under_the_same_id_int(
        self, label: str, my_trades_open: list
    ) -> dict:
        """
        my_trades_open = re-read from db after previous closed transaction was appended
        """

        transactions_under_same_id = self.transactions_same_id(label, my_trades_open)

        remaining_open_trades = self.remaining_open_trades(label, my_trades_open)

        return {
            "transactions_same_id": transactions_under_same_id["transactions_same_id"],
            # summing transaction under the same label id
            "transactions_same_id_contain_open_label": transactions_under_same_id[
                "transactions_same_id_contain_open_label"
            ],
            "transactions_same_id_net_qty": self.net_sum_trade_size(
                transactions_under_same_id["transactions_same_id"]
            ),
            "transactions_same_id_len": len(
                transactions_under_same_id["transactions_same_id"]
            ),
            "remaining_open_trades": str_mod.remove_redundant_elements(
    ...
    def remaining_open_trades(self, label: str, my_trades_open: list) -> None:
        """ """
        #
        return [
            o
    ...
    def closed_open_order_label_in_my_trades_open(
        ...
        label_trades = [
            str_mod.extract_integers_from_text(o["label"]) for o in my_trades_open
        ]

        return label in label_trades

    def synchronizing_closed_tradings(
        self, closed_label_id_int: str, my_trades_open: list, my_trades_path_open: str
    ) -> None:
        """ """
        ...
        closed_trades_in_my_trades_open = gather_transactions_under_the_same_id_int[
            "transactions_same_id"
        ]
        ...
        remaining_open_trades = gather_transactions_under_the_same_id_int[
            "remaining_open_trades"
        ]
        ...
        if gather_transactions_under_the_same_id_int[
            "transactions_same_id_contain_open_label"
        ]:
                    ...
                    for data_order in remaining_open_trades:
                        ...
                        if label_id != []:
                            ...
                            pickling.replace_data(
                                my_trades_path_open, remaining_open_trades, True
                            )
    ...
    def distribute_trade_transactions(
        self, currency: str, rebuilt: bool = False
    ) -> None:
        """
        ...
        try:
            my_trades_path_open = system_tools.provide_path_for_file(
                "myTrades", currency, "open"
            )
            numb = 0
            ...
            for data_order in my_trades:
                ...
                if label_id == []:
                    my_trades_path_manual = system_tools.provide_path_for_file(
                        "myTrades", currency, "manual"
                    )

                    pickling.append_data(my_trades_path_manual, data_order, True)
                ...
                if trade_transactions["closing_position"]:
                    ...
                    self.synchronizing_closed_tradings(
                        trade_transactions["closed_label_id_int"],
                        my_trades_open,
                        my_trades_path_open,
                    )
        ...
        try:
            ...
            if my_trades != []:
                ...
                return {
                    ...
                    "timestamp": ([o["timestamp"] for o in trade_list_with_max_price])[
                        0
                    ],
