...
strategy_attributes_for_hedging = [
    o for o in strategy_attributes if "hedgingSpot" in o["strategy"]
]
...
def test_are_size_and_order_appropriate_for_ordering():
    ...
    result = hedging.are_size_and_order_appropriate_for_ordering(
        notional, current_size, current_outstanding_order_len
    )
    assert result == True
...
def test_get_open_interest_symbol():
    ...
    result = hedging.hedged_value_to_notional(hedged_value, notional)
    assert result == 1
...
def test_price_pct():
    ...
    bid_price = 1858
    result = hedging.is_transaction_price_minus_below_threshold(
        price, bid_price, pct_threshold
    )
    assert result == True
    ...
    ask_price = 1974
    result = hedging.is_transaction_price_plus_above_threshold(
        price, ask_price, pct_threshold
    )
    assert result == True
...
def test_close_order():
    ...
    transaction = [
        {
            "trade_seq": 124324604,
            "trade_id": "ETH-168837663",
            "timestamp": 1682657827181,
            "tick_direction": 1,
            "state": "filled",
            "self_trade": False,
            "risk_reducing": False,
            ...
            "post_only": True,
            "order_type": "limit",
            "order_id": "ETH-33127542643",
            "mmp": False,
            "matching_id": None,
            "mark_price": 1916.47,
            "liquidity": "M",
            "label": "hedgingSpot-open-1682657827125",
    ...
    result = hedging.is_transaction_price_minus_below_threshold(
        last_transaction_price, bid_price, pct_threshold
    )
    assert result == True
    bid_price2 = 1859
    result = hedging.is_transaction_price_minus_below_threshold(
        last_transaction_price, bid_price2, pct_threshold
    )
    assert result == False
